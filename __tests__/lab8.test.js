...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let shadowRoot = await page.evaluateHandle(pi => pi.shadowRoot, productItem);
    let button = await shadowRoot.$('button');
    await button.click();
    let text = await page.evaluate(but => but.innerText, button);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let j = 0; j < productItems.length; j++){
      let shadowRoot = await page.evaluateHandle(pi => pi.shadowRoot, productItems[j]);
      let button = await shadowRoot.$('button');
      let buttonText = await page.evaluate(but => but.innerText, button);
      if(buttonText === 'Add to Cart'){
        await button.click();
      }
    }
    let cartCount = await page.$('#cart-count');
    let text = await page.evaluate(tex => tex.innerText, cartCount);
    expect(text).toBe("20");
  }, 10000);
